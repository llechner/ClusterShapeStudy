...
//  virtual void beginRun(Run&, EventSetup const&);
  ...
  double localPitch   (DetId id, LocalPoint p);
  int    layerwheel   (const edm::EventSetup* iSetup, uint32_t detid, uint32_t subdetid);

  const TrackerGeometry* m_tracker;
  std::map<DetId,double> m_thicknessMap;
  std::map<DetId,double> m_localPitchMap;
