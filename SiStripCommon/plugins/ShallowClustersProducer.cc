  ...
  : Prefix(iConfig.getParameter<std::string>("Prefix") )
{
  ...
  produces <std::vector<unsigned> >    ( Prefix + "amplitude"    );
  ...
  produces <std::vector<unsigned> >    ( Prefix + "clustercharge");
  ...
  produces <std::vector<unsigned> >    ( Prefix + "clusteridx"    );
...
produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  auto       amplitude     = std::make_unique<std::vector<unsigned>>();
  ...
  auto       clustercharge = std::make_unique<std::vector<unsigned>>();
  ...
  auto       clusteridx    = std::make_unique<std::vector<unsigned>>();
  ...
  for(;itClusters!=clusters->end();++itClusters){
    ...
    for(edmNew::DetSet<SiStripCluster>::const_iterator cluster=itClusters->begin(); cluster!=itClusters->end();++cluster){
      ...
      int clcharge = 0;
      for(unsigned int a=0;a<cluster->amplitudes().size();a++){
          clcharge+=cluster->amplitudes().at(a);
          if(cluster->amplitudes().at(a) >= 254)Saturation = true;
          amplitude->push_back( cluster->amplitudes().at(a) );
          stripidx->push_back( number->at(0) );
       }

      if(info.firstStrip()==0                                  )Overlapping=true;
      if(info.firstStrip()==128                                )Overlapping=true;
      if(info.firstStrip()==256                                )Overlapping=true;
      if(info.firstStrip()==384                                )Overlapping=true;
      if(info.firstStrip()==512                                )Overlapping=true;
      if(info.firstStrip()==640                                )Overlapping=true;

      if(info.firstStrip()<=127 && info.firstStrip()+cluster->amplitudes().size()>127)Overlapping=true;
      if(info.firstStrip()<=255 && info.firstStrip()+cluster->amplitudes().size()>255)Overlapping=true;
      if(info.firstStrip()<=383 && info.firstStrip()+cluster->amplitudes().size()>383)Overlapping=true;
      if(info.firstStrip()<=511 && info.firstStrip()+cluster->amplitudes().size()>511)Overlapping=true;
      if(info.firstStrip()<=639 && info.firstStrip()+cluster->amplitudes().size()>639)Overlapping=true;

      if(info.firstStrip()+cluster->amplitudes().size()==127                   )Overlapping=true;
      if(info.firstStrip()+cluster->amplitudes().size()==255                   )Overlapping=true;
      if(info.firstStrip()+cluster->amplitudes().size()==383                   )Overlapping=true;
      if(info.firstStrip()+cluster->amplitudes().size()==511                   )Overlapping=true;
      if(info.firstStrip()+cluster->amplitudes().size()==639                   )Overlapping=true;
      if(info.firstStrip()+cluster->amplitudes().size()==767                   )Overlapping=true;


      overlapping->push_back(  Overlapping                                             );
      saturation->push_back(   Saturation                                              );
      clusteridx->push_back(   number->at(0)                                           );
      ...
      clustercharge->push_back( clcharge                                               );
  ...
  iEvent.put(std::move(amplitude),   Prefix + "amplitude"    );
  ...
  iEvent.put(std::move(clustercharge), Prefix + "clustercharge" );
  ...
  iEvent.put(std::move(clusteridx),   Prefix + "clusteridx"    );
